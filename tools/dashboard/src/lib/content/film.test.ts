...
describe("parseFilmScript", () => {
  it("pulls a reel apart into the parts you film from", () => {
    ...
    expect(f.shots).toEqual(["Brett to camera: the hook", "The terminal with the error rows"]);
  });
...
describe("toLines", () => {
  ...
  it("does not split a decimal, an abbreviation mid-sentence, or a url", () => {
    ...
    expect(toLines("Go to os.settoku.app now.")).toEqual(["Go to os.settoku.app now."]);
  });
