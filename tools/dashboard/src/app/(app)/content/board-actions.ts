...
export async function moveCard(
  ...
  // drawer can retry — it must NEVER turn a saved move into a failure (Brett: never block the board).
...
export async function rejectScript(id: string, reason: string): Promise<MoveResult> {
  ...
  const header = existing
    ...
    : [
        `# ${FEEDBACK_DOC}`,
        "",
        "Every script Brett binned, and the reason in his own words. The writer reads the newest of",
        "these before choosing an angle and again before writing, so this file is how his taste",
        "reaches the machine. Edit or delete anything here that no longer reflects what he wants.",
        "",
      ].join("\n");
